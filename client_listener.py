#TODO: expose port for connector client to join
...
#not used anymore since this is set listener client
...
def listener_for_peer(ip, port, event):
    global peer_socket
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
...
t = threading.Thread(target=listener_for_peer, args=(listener_ip, listener_port, peer_connected_event)) 
t.start()
